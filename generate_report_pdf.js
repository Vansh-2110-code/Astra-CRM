...
// Table Header
const startY = doc.y;
doc.rect(40, startY, 515, 24).fill('#4338ca');

doc.fillColor('#ffffff')
   .fontSize(9)
   .font('Helvetica-Bold');
doc.text('Component Layer', 48, startY + 7, { width: 140 });
doc.text('Verification Status', 195, startY + 7, { width: 110 });
doc.text('Technical Assessment & Details', 310, startY + 7, { width: 235 });
...
const rows = [
  ...
  },
  {
    ...
    status: 'VERIFIED & PASSED',
    details: 'POST /api/payments/razorpay/order creates orders in paise (₹3,000 / ₹5,000).'
  },
  {
  ...
  },
  {
    ...
    status: 'VERIFIED & PASSED',
    details: 'PricingPlansModal handles 15 seats (₹3,000), 25 seats (₹5,000) & Contact Us.'
  },
  {
...
rows.forEach((row, i) => {
  ...
  doc.fillColor('#111827')
     .fontSize(8.5)
     .font('Helvetica-Bold')
     .text(row.component, 48, currentY + 7, { width: 140 });

  doc.fillColor('#059669')
     .fontSize(8)
     .font('Helvetica-Bold')
     .text(`[PASSED] ${row.status}`, 195, currentY + 7, { width: 110 });

  doc.fillColor('#4b5563')
     .fontSize(8)
     .font('Helvetica')
     .text(row.details, 310, currentY + 7, { width: 235 });
...
plans.forEach(p => {
  doc.fillColor('#4338ca').fontSize(10).font('Helvetica-Bold').text(`• ${p.title} - `, { continued: true });
  doc.fillColor('#111827').font('Helvetica-Bold').text(`${p.price} (${p.seats})`, { continued: false });
  doc.fillColor('#4b5563').fontSize(8.5).font('Helvetica').text(`   ${p.desc}`);
  doc.moveDown(0.4);
});
